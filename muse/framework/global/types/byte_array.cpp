...
bool muse::ByteArray::operator==(const ByteArray& other) const {
    if (size() != other.size()) {
        return false;
    }
...
ByteArray& ByteArray::insert(size_t pos, uint8_t byte) {
    if (pos > size()) {
        return *this;
    }

    detach();
    m_data->insert(m_data->begin() + pos, byte);

    return *this;
}

void ByteArray::push_back(uint8_t byte) {
    insert(size(), byte);
}

void ByteArray::push_back(const uint8_t* byte, size_t len) {
    detach();
    size_t start = size();
...
void ByteArray::push_back(const ByteArray& byteArray) {
    push_back(byteArray.constData(), byteArray.size());
}
